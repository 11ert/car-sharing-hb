...
public class MemberBean implements Serializable
{
   ...
   public void paginate()
   {
      ...
      TypedQuery<Member> query = this.entityManager.createQuery(criteria
   ...
   private Predicate[] getSearchPredicates(Root<Member> root)
   {
      ...
      int carsize = this.example.getCarsize();
      if (carsize != 0)
      {
         predicatesList.add(builder.equal(root.get("carsize"), carsize));
      }
   ...
   public List<Member> getAll()
   {
      ...
      CriteriaQuery<Member> criteria = this.entityManager
            .getCriteriaBuilder().createQuery(Member.class);
      return this.entityManager.createQuery(
            criteria.select(criteria.from(Member.class))).getResultList();
   }
