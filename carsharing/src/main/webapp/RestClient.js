...
// Model for Member entity
    models.Member = Backbone.Model.extend({
        urlRoot: "http://localhost:8080/carsharing/rest/members/",
        idAttribute: 'id',
        defaults: {
            firstname: "",
            phoneNumber: "",
        ...
        sync: function (method, model, options) {
            ...
            if (method == 'create') {
                options.url = 'http://localhost:8080/carsharing/rest/members/';
            }
            if (method == 'update') {
                return false;
            }
    ...
    // Collection class for Member entities
    models.MemberCollection = Backbone.Collection.extend({
        model: models.Member,
        url: "http://localhost:8080/carsharing/rest/members/",
        sync: function (method, model, options) {
            options || (options = {});
            var errorHandler = {
                error: function (jqXHR, textStatus, errorThrown) {
...
$(function () {
    ...
    var AppRouter = Backbone.Router.extend({
        ...
        list: function () {
            this.collection = new models.MemberCollection();
            var self = this;
            this.collection.fetch({
                success: function () {
                    self.listView = new views.ListView({
                        model: self.collection,
                        // tpl-member-list-itemis template identifier for item
                        templateName: '#tpl-member-list-item'
                    });
        ...
        details: function (id) {
            if (this.collection) {
                this.member = this.collection.get(id);
                if (this.view) {
                    this.view.close();
                }
                var self = this;
                this.view = new views.ModelView({
                    model: this.member,
                    // tpl-member-details is template identifier for chosen model element
                    templateName: '#tpl-member-details',
        ...
        create: function () {
            ...
            var self = this;
            var dataModel = new models.Member();
            // see isNew() method implementation in the model
            dataModel.notSynced = true;
            this.view = new views.ModelView({
                model: dataModel,
                collection: this.collection,
                // tpl-member-details is a template identifier for chosen model element
                templateName: '#tpl-member-details',
